#!/usr/bin/env python3
"""Standalone LoRA trainer for multimodal Qwen3.5-4B (text-only).
...
from core.train import target_text, log, vram_mb
...
def main() -> None:
    ...
    # LoRA on the LM backbone q/v projections.
    ...
    _ta = dict(
        ...
        eval_strategy="no", save_strategy="steps", save_total_limit=int(train_cfg["save_total_limit"]), load_best_model_at_end=False,
    ...
    log("starting training (eval=skipped)...")
    start = time.time()
